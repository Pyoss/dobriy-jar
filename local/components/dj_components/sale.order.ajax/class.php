...
class SaleOrderAjax extends \CBitrixComponent
{
    ...
    protected function getUserName(){
        ...
        $this->arResult['USER_NAME'] = $USER -> GetFirstName() ?: $USER ->GetLogin();
	...
	protected function getInnerPaySystemInfo(Order $order, $recalculate = false)
	{
        ...
        $arResult['ALL_BUDGET_FORMATED'] = SaleFormatCurrency(round((float)$arResult['USER_ACCOUNT']['CURRENT_BUDGET']), $order->getCurrency());

		if ($order->isAllowPay() && $this -> arUserResult['PAY_CURRENT_ACCOUNT'] === 'Y')
		{
			$innerPaySystemId = PaySystem\Manager::getInnerPaySystemId();
            ...
            */
            $payLimit = $order->getBasket()->getPrice() * 0.3;
			$sumToSpend = round($userBudget >= $payLimit ? $payLimit : $userBudget);
		...
		if ($sumToSpend > 0)
		{
			...
			$arResult['CURRENT_BUDGET_FORMATED'] = SaleFormatCurrency($sumToSpend, $order->getCurrency());
		}
		else
		{
	...
	protected function getJsDataResult()
	{
        ...
        if ($arResult['USER_NAME']){
            $result["USER_NAME"] = $arResult['USER_NAME'];
            $result["USER_LAST_NAME"] = $arResult['USER_LAST_NAME'];
        }
        ...
        $result['ALL_BUDGET_FORMATED'] = $arResult["ALL_BUDGET_FORMATED"];
	...
	protected function makeUserResultArray()
	{
		...
		if ($request->isPost())
		{
			...
			if (empty($arUserResult["PERSON_TYPE_OLD"]) || $arUserResult["PERSON_TYPE_OLD"] == $arUserResult["PERSON_TYPE_ID"])
			{
				...
				if ($request->get('PAY_CURRENT_ACCOUNT') == "on")
