    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        saveOrder: function (result) {
            ...
            if (res && res.order) {
                ...
                this.result.order_id = result.ID
        ...
        editSection: function (section) {
            ...
            switch (section.id) {
                ...
                // Убираем корзину так как добавляем на эту же страницу модуль корзины
                //case this.basketBlockNode.id:
        ...
        isSelectedInnerPayment: function () {
            var innerPaySystemCheckbox = this.paySystemBlockNode.querySelector('input[type=radio][name=PAY_CURRENT_ACCOUNT]');

            if (!innerPaySystemCheckbox)
                innerPaySystemCheckbox = this.paySystemHiddenBlockNode.querySelector('input[type=radio][name=PAY_CURRENT_ACCOUNT]');
        ...
        selectPaySystem: function (event) {
            ...
            var target = event.target || event.srcElement,
                ...
                innerPaySystemCheckbox = this.paySystemBlockNode.querySelector('input[type=radio][name=PAY_CURRENT_ACCOUNT]'),
        ...
        createDeliveryItem: function (item) {
            var checked = item.CHECKED == 'Y',
                ...
                price, label, title, itemNode, desc;


            if (typeof item.DESCRIPTION !== 'undefined') {
                desc = (
                    BX.create('DIV', {
                        props: {className: 'bx-soa-pp-delivery-info'},
        ...
        editActivePropsBlock: function (activeNodeMode) {
            ...
            if (this.initialized.props) {
                ...
                /*
                this.commentBlockNode = BX.create('DIV', {props: {id: 'commentBlock'}})
                propsNode.appendChild(this.commentBlockNode);
        ...
        editPropsItems: function (propsNode) {
            ...
            var propsItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-customer'}}),
                group, property, propertySettings, containerToInsert,
                groupIterator = this.propertyCollection.getGroupIterator(), propsIterator;
        ...
        getPropertyRowNode: function (property, propsItemsContainer, disabled) {
            ...
            if (disabled) {
                ...
                let classname = 'bx-soa-custom-label'
        ...
        getValidationData: function (arProperty, propContainer) {
            ...
            switch (arProperty.TYPE) {
                ...
                case 'LOCATION':
                    ...
                    break;
                case 'Y/N':
                    data.inputs = propContainer.querySelectorAll('input[type=radio]');
                    data.action = 'change';
                    data.func = BX.delegate(function (input, fieldName) {
        ...
        editTotalBlock: function () {
            ...
            */
            ...
            if (this.options.showDiscountPrice) {
                ...
                this.totalInfoBlockNode.appendChild(this.createTotalUnit('Скидка:', '-' + total.DISCOUNT_PRICE_FORMATED, {highlighted: true}));
            }

            if (this.options.showPayedFromInnerBudget) {
                this.totalInfoBlockNode.appendChild(this.createTotalUnit('Оплачено баллами:', '-' + total.PAYED_FROM_ACCOUNT_FORMATED, {highlighted: true}));
                this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_IT'), total.ORDER_TOTAL_LEFT_TO_PAY_FORMATED, {total: true}));
            } else {
            ...
            if (this.result.IS_AUTHORIZED) {
                return BX.create('DIV', {
                    ...
                    children: [
                        BX.create('DIV', {
                            props: {
                                className: 'bx-soa-auth-name'
                            },
                            text: this.result.USER_NAME + ' ' +
                                (!!this.result.USER_LAST_NAME ?this.result.USER_LAST_NAME: ' ')
                        }),
                        BX.create('DIV', {
                            props: {
                                className: 'bx-soa-bonus-amount'
                            },
                            children: [BX.create('SPAN', {
                                ...
                                BX.create('SPAN', {
                                    props: {
                                        className: 'bx-soa-bonus-formatted'
                                    },
                                    html: this.result.ALL_BUDGET_FORMATED
                                })
                            ]
                        }),
                        BX.create('LABEL', {
                            props: {className: 'bx-bonus-points-label'},
                            children: [
                                BX.create('INPUT', {
                                    props: {
                                        type: 'checkbox',
                                        name: 'PAY_CURRENT_ACCOUNT',
                                        checked: this.result.PAY_CURRENT_ACCOUNT === 'Y',
                                        className: 'bx-bonus-points-input',
                                    },
                                ...
                                BX.create('SPAN', {
                                    props: {className: 'bx-bonus-points-ltext'},
                ...
                return BX.create('div', {
                    attrs: {className: 'bx-soa-bonus-block'},
                    html: '<a class="dj_link" href="/auth/">Зарегистрируйтесь на сайте</a>, чтобы получить 100 бонусных баллов на оплату товаров.'
                })
        ...
        editMobileTotalBlock: function () {
            ...
            this.mobileTotalBlockNode.appendChild(this.editBonusAuthBlock())
            this.mobileTotalBlockNode.appendChild(this.totalInfoBlockNode.cloneNode(true));
            this.editCoupons(this.mobileTotalBlockNode.lastChild)
        ...
        setAnalyticsDataLayer: function () {
            ym(61893639, 'reachGoal', 'goal_make_order')
            let result_products = []
            for (let product in this.result.GRID.ROWS){
                let product_data = this.result.GRID.ROWS[product].data
                result_products.push(
                    {
                        id: product_data.PROPERTY_ARTNUMBER_VALUE,
