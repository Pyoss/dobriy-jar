	...
	foreach($arCategory["ITEMS"] as $i => $arItem)
	{
		if(isset($arItem["ITEM_ID"]))
		{
			...
			elseif (
                ...
                $arResult["FOUND_CATEGORIES"][$arItem['ITEM_ID']] = ltrim($arItem["ITEM_ID"], 'S');
            }
		}
	}
}
...
if (!empty($arResult["FOUND_CATEGORIES"]) && CModule::IncludeModule("iblock")) {
    ...
    $arFilter = array(
        "ACTIVE" => "Y",
        "=ID" => array_values($arResult["FOUND_CATEGORIES"]),
    );
    $rsSection = SectionTable::GetList(array('filter' => $arFilter, 'select' => $arSelect));
    while($arSection = $rsSection->Fetch()) {
        $list = CIBlockSection::GetNavChain(false, $arSection["ID"], array('NAME'), true);
        for ($i = 0; $i < (count($list) - 1); $i++) {
            $arSection['NAME_PATH'][] = $list[$i]['NAME'];
        }
        $arResult["FOUND_CATEGORIES"]['S'.$arSection["ID"]] = $arSection;
			...
			if(array_key_exists($arItem["ITEM_ID"], $arResult["ELEMENTS"]))
			{
				...
				if ($arParams["SHOW_PREVIEW"] == "Y")
				{
					...
					elseif ($arElement["DETAIL_PICTURE"] > 0)
                        ...
                        $arSKU = CCatalogSKU::getOffersList($arItem["ITEM_ID"], 2, array('ACTIVE' => 'Y'), array('PREVIEW_PICTURE', 'SORT'));
                        usort($arSKU[$arItem["ITEM_ID"]], function ($item1, $item2) {
                            return $item1['SORT'] <=> $item2['SORT'];
                        });
                        $pict = $arSKU[$arItem["ITEM_ID"]][0]['PREVIEW_PICTURE'];
                        if($pict){
                            $arElement["PICTURE"] = CFile::ResizeImageGet($pict, array("width"=>$PREVIEW_WIDTH, "height"=>$PREVIEW_HEIGHT), BX_RESIZE_IMAGE_PROPORTIONAL, true);
                        } else {
                            $arElement["PICTURE"]["src"] = '/images/template.png';
                            $arElement["PICTURE"]["width"] = $PREVIEW_WIDTH;
                            $arElement["PICTURE"]["height"] = $PREVIEW_HEIGHT;

                        }
					}
				}
			}
            elseif(array_key_exists($arItem["ITEM_ID"], $arResult["FOUND_CATEGORIES"]))
            {
                $arElement = &$arResult["FOUND_CATEGORIES"][$arItem["ITEM_ID"]];
